...
use led_control::{Pixel, Effect, AuxEffect, set_effect, setup};
...
fn main() {
    ...
    let b: Arc<AtomicBox<Option<(Pixel, Effect, Option<Pixel>, AuxEffect)>>> =
        Arc::new(AtomicBox::new(None));
    let b2 = b.clone();
    ...
    for l in stdin.lock().lines().filter_map(|x| x.ok()).filter_map(
        string_to_opts,
    )
    {
        println!("{:?}", l);
