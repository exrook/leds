...
impl OutputDescriptor {
    ...
    pub fn from(ptr: *const CxxOutputDescriptor) -> Self {
        let identifier = unsafe {
            CStr::from_ptr(
                cpp!([ptr as "Vamp::Plugin::OutputDescriptor*"] -> *const c_char as "const char*" {
        ...
        let has_fixed_bin_count = unsafe {
            cpp!([ptr as "Vamp::Plugin::OutputDescriptor*"] -> bool as "bool" {
        ...
        let bin_count = match has_fixed_bin_count {
            true => {
                Some(unsafe {
                    cpp!([ptr as "Vamp::Plugin::OutputDescriptor*"] -> usize as "size_t" {
                    return ptr->binCount;
                })
                })
            }
        ...
        let bin_names = match has_fixed_bin_count {
            true => {
                let cxx_vec: CxxVector<CxxString> = unsafe {
                    CxxVector::from(
                        cpp!([ptr as "Vamp::Plugin::OutputDescriptor*"] -> *mut CxxInnerVector as "const std::vector<std::string>*" {
                    return &(ptr->binNames);
                }),
                    )
                };
        ...
        let min_value = match has_known_extents {
            true => {
                Some(unsafe {
                    cpp!([ptr as "Vamp::Plugin::OutputDescriptor*"] -> f32 as "float" {
                    return ptr->minValue;
                })
                })
            }
        ...
        let sample_type = match unsafe {
            cpp!([ptr as "Vamp::Plugin::OutputDescriptor*"] -> i32 as "int" {
